...
ProcessStats stats;
double globalMaxIterTime;
double globalMinIterTime;
...
void *responseThread(void *arg)
{
    ...
    while (!allTasksDone)
    {
        ...
        if (flag)
        {
            ...
            {
                int numTasksToSend = std::min(5, availableTasks / 3);
...
int main(int argc, char **argv)
{
    ...
    MPI_Datatype statsTypes[4] = {MPI_INT, MPI_DOUBLE, MPI_INT, MPI_INT};
    int statsBlocklengths[4] = {1, 1, 1, 1};
    MPI_Aint statsOffsets[4];
    statsOffsets[0] = offsetof(ProcessStats, tasksProcessed);
    statsOffsets[1] = offsetof(ProcessStats, workTime);
    statsOffsets[2] = offsetof(ProcessStats, tasksReceived);
    statsOffsets[3] = offsetof(ProcessStats, tasksSent);
    MPI_Type_create_struct(4, statsBlocklengths, statsOffsets, statsTypes, &MPI_STATS_TYPE);
    ...
    for (int iterCounter = 0; iterCounter < NUM_ITERATIONS; ++iterCounter)
    {

        pthread_mutex_lock(&taskMutex);
        taskList.clear();
        for (int i = 0; i < NUM_TASKS / size; ++i)
        {
            // int weight = 5000 + abs(rank - (iterCounter % size)) * 500;
            int weight = abs(50 - i % NUM_TASKS) * abs(rank - (iterCounter % size)) * L;
        ...
        MPI_Reduce(&stats.workTime, &globalMaxIterTime, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        MPI_Reduce(&stats.workTime, &globalMinIterTime, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);

        if (rank == 0)
        {
            double imbalanceTime = globalMaxIterTime - globalMinIterTime;
            double imbalanceShare = (imbalanceTime / globalMaxIterTime) * 100;
            std::cout << "Итерация " << iterCounter << " завершена за "
                      << std::fixed << std::setprecision(3) << iterTime << " секунд"
                      << " время дисбаланса " << imbalanceTime
                      << " доля дисбаланса " << imbalanceShare
                      << " результат " << globalRes
                      << std::endl;
        }
        globalRes = 0;
...
void printFinalStats()
{
    ...
    if (rank == 0)
    {
        ...
        std::cout << "Ранг | Заданий | Время работы | Получено | Отправлено" << std::endl;
        std::cout << "---------------------------------------------------------------" << std::endl;
